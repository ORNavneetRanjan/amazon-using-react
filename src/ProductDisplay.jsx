...
function ProductDisplay({onAddToCart}) {
  ...
  return (
    ...
    <div className="grow bg-gray-200 p-4 md:p-8 justify-center items-center flex flex-col  min-w-screen gap-5">
      <Link to="/" className="bg-sky-500 text-white text-lg md:text-xl lg:text-2xl rounded-md mb-2 ml-4 md:ml-24 p-2 self-start">
        <IoChevronBackSharp />
      </Link>
      <div className="grow bg-white flex flex-col md:flex-row p-4 md:p-5 shadow-xl mx-4 md:mx-14 w-full max-w-4xl">
        ...
        <div className="flex flex-col px-2 md:px-8 gap-2 md:gap-5 w-full md:w-1/2 mt-4 md:mt-0">
          ...
          <h2 className="font-bold text-xl md:text-2xl">Price: ${product.price}</h2>
          ...
          <div className="flex gap-2 md:gap-5">
            ...
            />
            <button onClick={handleButtonClick}  className="bg-red-500 text-white p-1 md:p-2 self-center rounded-lg text-lg md:text-2xl">
              ADD TO CART
            </button>
          </div>
        </div>
      ...
      <div className='flex justify-between px-4 lg:px-24 w-full'>
        {sku > 1 ? (
          <Link to={"/product/" + (sku - 1)} className="bg-sky-500 text-white text-lg lg:text-2xl rounded-md p-2 lg:p-5">
            <RxTrackPrevious />
          </Link>
        ) : (
